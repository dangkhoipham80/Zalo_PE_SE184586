...
import com.example.zalo_pe_se184586.adapter.SelectContactsAdapter;
...
import com.example.zalo_pe_se184586.vm.SelectContactsViewModel;
...
public class SelectContactsActivity extends AppCompatActivity implements SelectContactsAdapter.OnContactToggleListener {
    ...
    private void createGroup(String groupName) {
        List<String> selectedIds = viewModel.getSelectedContactIdsSnapshot();
        if (selectedIds.size() < 2) {
            Toast.makeText(this, R.string.error_min_members, Toast.LENGTH_SHORT).show();
            return;
        }
        List<String> memberIds = new ArrayList<>();
        memberIds.add(contactRepository.getCurrentUser().getId());
        for (String id : selectedIds) {
            if (!memberIds.contains(id)) {
                memberIds.add(id);
            }
        }
        String groupId = groupRepository.createGroup(groupName, memberIds).getId();
